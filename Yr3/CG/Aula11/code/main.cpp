...
void computeNormal(int i, int j, std::vector<float>& normal) {
	...
	float p1[3] = {i,h(i,(j-1)),j-1};
	float p2[3] = {i,h(i,(j+1)),j+1};
	float p3[3] = {i-1,h((i-1),j),j};
	float p4[3] = {i+1,h((i+1),j),j-1};
...
void prepareTerrain() {
	...
	for (int i = 1; i < imageWidth - 2; i++) {
		for (int j = 1; j < imageWidth - 1; j++) {
			...
			// to do: fill texCoord vector with the texture coordinates for grid location (i+1,j)
			...
			// to do: fill texCoord vector with the texture coordinates for grid location (i,j)
	...
	// to do: copy data from texCoord and normal to OpenGL buffers
	...
	// to do: Enable texCoord and normal buffers
